...
const Dashboard = () => {
  ...
  const fetchTotalUsers = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/api/total-users"
      );
  ...
  const fetchTotalDrivers = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/api/total-drivers"
      );
      const data = await response.json();
  ...
  const fetchTotalPassenger = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/api/total-passenger"
      );
      const data = await response.json();
  ...
  const fetchTotalCar = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/api/total-cars"
      );
      const data = await response.json();
  ...
  return (
    <div className="dashboard">
      <div className="grid-container">
        <div
          className="grid-item"
          style={{ backgroundColor: "rgb(245, 53, 171)" }}
        >
