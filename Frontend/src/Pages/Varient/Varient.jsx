...
const VarientList = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/Variants"
      );
      if (!response.ok) {
  ...
  const fetchDataAll = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/AllVariants"
      );
      if (!response.ok) {
        throw new Error("Failed to fetch data");
      }
    ...
    try {
      if (key) {
        let result = await fetch(
          `https://admin-live-project.onrender.com/searchVariant/${key}`
        );
        if (!result.ok) {
  ...
  const handleModalOk = async () => {
    try {
      await fetch(
        `https://admin-live-project.onrender.com/Varient/${deleteCarId}`,
        {
          method: "DELETE",
        }
      );
    ...
    <div className="home">
      ...
      >
        ...
        <div className="addbtn">
          <Button type="primary">
            <Link to="/NewVarient">+ Add New Variant</Link>{" "}
