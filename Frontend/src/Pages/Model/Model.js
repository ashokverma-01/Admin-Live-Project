...
const Model = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/models"
      );
      if (!response.ok) {
  ...
  const fetchDataAll = async () => {
    try {
      const response = await fetch(
        "https://admin-live-project.onrender.com/Allmodels"
      );
      if (!response.ok) {
        throw new Error("Failed to fetch data");
      }
    ...
    try {
      if (key) {
        let result = await fetch(
          `https://admin-live-project.onrender.com/searchModel/${key}`
        );
        if (!result.ok) {
  ...
  const handleModalOk = async () => {
    try {
      await fetch(
        `https://admin-live-project.onrender.com/Model/${deleteCarId}`,
        {
          method: "DELETE",
        }
      );
